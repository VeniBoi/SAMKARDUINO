...
{
	public Text ennenLaskua, laskunJalkeen;
	...
	void Update()
	{
		transform.position = new Vector3(transform.position.x, (float)-BasicDemo.S6 / 10 / 10, transform.position.z);
		CheckSurrounding();
		ennenLaskua.text = "S6 Arvo ennen laskua: " + (float)BasicDemo.S6;
		laskunJalkeen.text = "S6 Arvo laskun jalkeen: " + (float)-BasicDemo.S6 / 10 / 10;
	}
